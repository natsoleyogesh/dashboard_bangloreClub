...
const AddMemberApplication = () => {
    ...
    const validateForm = () => {
        ...
        // Check if sponsoredBy is empty
        if (sponsoredBy.length === 0) {
            errors.sponsoredBy = "At least one sponsor is required.";
        }
    ...
    const handleSubmit = async (event) => {
        ...
        // Add selected sponsors to the formData
        formData.append("sponsoredBy", JSON.stringify(sponsoredBy));

        try {
            const response = await addMemberApplication(formData);
            if (response.status === 201) {
                showToast("Member application added successfully!", "success");
                navigate("/customers");
            } else {
                            ...
                            <Grid container spacing={2}>
                                ...
                                {/* <Grid item xs={12}>
                                    ...
                                    <FormControl fullWidth margin="dense">
                                        <Autocomplete
                                            multiple
                                            value={sponsoredBy}
                                            onChange={(event, newValue) => setSponsoredBy(newValue)}
                                            options={memberList}
                                            ...
                                            renderInput={(params) => (
                                                <TextField
                                                    ...
                                                    margin="normal"
                                                    error={!!validationErrors.sponsoredBy} // Show error if validation fails
                                                    helperText={validationErrors.sponsoredBy} // Display error message
                                                />
                                            )}
                                ...
                                <Grid item xs={12}>
                                    ...
                                    <FormControl fullWidth margin="dense">
                                        <Autocomplete
                                            multiple
                                            value={memberList.filter(member => sponsoredBy.includes(member._id))}
                                            onChange={(event, newValue) => {
                                                // Extract only the _id from the selected options and update the sponsoredBy state
                                                setSponsoredBy(newValue.map(option => option._id));
                                            }}
                                            options={memberList}
                                            getOptionLabel={(option) => {
                                                // Return a string formatted as "name (memberId)"
                                                return `${option.name} (${option.memberId})`;
                                            }}
                                            renderInput={(params) => (
                                                <TextField
                                                    {...params}
                                                    ...
                                                    placeholder="Search by name or memberId"
                                                    margin="normal"
                                                    error={!!validationErrors.sponsoredBy}
                                                    helperText={validationErrors.sponsoredBy}
                                                />
                                            )}
                                            isOptionEqualToValue={(option, value) => option._id === value}  // Compare using _id
                                            renderOption={(props, option, { selected }) => (
                                                <li {...props}>
                                                    <Checkbox
                                                        checked={selected}
                                                        disabled={sponsoredBy.includes(option._id)} // Disable if already selected
                                                    />
                                                    {`${option.name} (${option.memberId})`}
                                                </li>
                                            )}
